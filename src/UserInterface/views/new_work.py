...
class NewWorkView:
    ...
    def _show_no_work_assigned_screen(self) -> None:
        ...
        # Icon
        icon = ctk.CTkLabel(frame, text="🚫", font=("Segoe UI", 72))
        icon.pack(pady=(50, 20))

        # Title
        title = ctk.CTkLabel(
            frame, text="No Work Assigned", font=("Segoe UI", 24, "bold")
        )
        title.pack(pady=10)
    ...
    def validate_data(self) -> ValidationResult:
        ...
        self.validation_result = self.data_validator.validate_equipment_map(
            self.state.equipment_map, self.state.file_to_textboxes
        )
    ...
    def _highlight_empty_cells(self) -> None:
        ...
        empty_indices = self.data_validator.get_empty_cell_indices(
            self.state.file_to_textboxes, self.validation_result.empty_cells
        )
    ...
    def rebuild_data_tables(self) -> None:
        ...
        if (
            not hasattr(self, "files_edit_container")
            or self.files_edit_container is None
        ):
            return
    ...
    def _build_file_table(
        self, file_path: str, file_idx: int, columns: List[tuple]
    ) -> None:
        ...
        file_section = ctk.CTkFrame(
            self.files_edit_container, fg_color=Colors.TRANSPARENT
        )
    ...
    def _build_table_header(self, parent: ctk.CTkFrame, columns: List[tuple]) -> None:
        ...
        header_row = ctk.CTkFrame(
            parent, fg_color=Colors.TABLE_HEADER_BG, corner_radius=0
        )
    ...
    def save_to_excel(self) -> None:
        ...
        with LoadingContext(
            self.controller, "Checking for changes...", show_progress=True
        ) as loading:
            ...
            if not updated_equipment:
                messagebox.showinfo(
                    "No Changes", "No changes were made to the equipment data."
                )
                return

            loading.update_progress(
                0.5, f"Saving {len(updated_equipment)} equipment items..."
            )
            ...
            for row_idx in range(num_rows):
                ...
                if equipment_no and equipment_no in self.state.equipment_map and parts:
                    ...
                    has_changes = self._check_row_for_changes(
                        equipment_no, parts, row_entries
                    )
                    ...
                    if has_changes and equipment_no not in changed_equipment:
                        ...
                        self._apply_row_changes(
                            changed_equipment[equipment_no], parts, row_entries
                        )

        return changed_equipment

    def _check_row_for_changes(
        self, equipment_no: str, parts: str, row_entries: list
    ) -> bool:
        ...
        for component in equipment.components:
            if component.component_name == parts:
                ...
                for key, ui_value in ui_values.items():
                    if ui_value:
                        current_value = str(
                            component.get_existing_data_value(key) or ""
                        )
    ...
    def _run_save(self, updated_equipment: Dict[str, object]) -> None:
        ...
        try:
            ...
            success = self.equipment_service.save_equipment_data(
                updated_equipment, self.workpathname
            )
            ...
            if success:
                    ...
                    for eq_no, equipment in updated_equipment.items():
                        ...
                        if db_equipment:
                            ...
                            for component in equipment.components:
                                ...
                                if original:
                                    orig_comp = original.get_component(
                                        component.component_name
                                    )
                                    if orig_comp:
                                        to_fill, corrected = (
                                            DatabaseService.count_correction_fields(
                                                orig_comp, component.existing_data
                                            )
                                        )
                ...
                self.parent.after(
                    0, lambda: messagebox.showerror("Save Failed", Messages.SAVE_FAILED)
                )
            ...
            self.parent.after(
                0,
                lambda: messagebox.showerror(
                    "Save Error", f"Error saving data: {str(e)}"
                ),
            )
