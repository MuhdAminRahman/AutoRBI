...
class RegistrationView:
    ...
    def show(self) -> None:
        ...
        strength_container = ctk.CTkFrame(fields, fg_color="transparent")
        strength_container.pack(fill="x", pady=(0, 4))
        strength_container.grid_columnconfigure(0, weight=1)

        # Password strength progress bar
        strength_bar_bg = ctk.CTkFrame(
            strength_container,
            height=6,
            corner_radius=3,
            fg_color=("#E0E0E0", "#404040"),
        )
        strength_bar_bg.grid(row=0, column=0, sticky="ew")
        strength_bar_bg.grid_propagate(False)
        ...
        strength_label.grid(row=1, column=0, sticky="w", pady=(4, 0))

        def update_password_strength(event=None):
            """Update password strength indicator based on password input."""
            password = password_entry.get()
            
            if not password:
                # Empty password
                strength_bar.place_configure(relwidth=0)
            ...
            # Update progress bar
            strength_bar.place_configure(relwidth=width)
            strength_bar.configure(fg_color=color)
